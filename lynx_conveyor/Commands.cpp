...
bool cmd_motor(char *args, CommandCaller *caller)
{
    ...
    int actionCode = 0;
    if (strcmp(action, "init") == 0) actionCode = 1;
    else if (strcmp(action, "status") == 0) actionCode = 2;
    else if (strcmp(action, "clear") == 0) actionCode = 3;
    else if (strcmp(action, "home") == 0) actionCode = 4;
    else if (strcmp(action, "abort") == 0) actionCode = 5;
    else if (strcmp(action, "stop") == 0) actionCode = 6;
    ...
    switch (actionCode) {
        case 1: { // init
            ...
            switch (motorState) {
                case MOTOR_STATE_IDLE:        caller->println(F("IDLE")); break;
                case MOTOR_STATE_MOVING:      caller->println(F("MOVING")); break;
                case MOTOR_STATE_HOMING:      caller->println(F("HOMING")); break;
                case MOTOR_STATE_FAULTED:     caller->println(F("FAULTED")); break;
                case MOTOR_STATE_NOT_READY:   caller->println(F("NOT READY")); break;
                default:                      caller->println(F("UNKNOWN")); break;
            }
        ...
        case 2: { // status
            ...
            caller->print(F("  Position: "));
            double calculatedPositionMm = pulsesToMm(MOTOR_CONNECTOR.PositionRefCommanded());
            caller->print(calculatedPositionMm, 2);
            caller->println(F(" mm"));
            
            // Display raw position in pulses
            caller->print(F("  Raw position: "));
            caller->print(MOTOR_CONNECTOR.PositionRefCommanded());
            caller->println(F(" pulses"));
            
            // Display velocity settings
            caller->print(F("  Velocity: "));
...
bool cmd_jog(char *args, CommandCaller *caller)
{
    ...
    int actionCode = 0;
    if (strcmp(action, "+") == 0) actionCode = 1;
    else if (strcmp(action, "-") == 0) actionCode = 2;
    else if (strcmp(action, "inc") == 0) actionCode = 3;
    else if (strcmp(action, "speed") == 0) actionCode = 4;
    else if (strcmp(action, "status") == 0) actionCode = 5;
    ...
    switch (actionCode) {
        case 1: { // jog forward (+)
            ...
            if (jogMotor(true)) {  // true = forward direction
        ...
        case 2: { // jog backward (-)
            ...
            // Perform the jog movement using the jogMotor function
            if (jogMotor(false)) {  // false = backward direction
                caller->println(F("[MESSAGE] Jog movement initiated"));
                return true;
            } else {
                caller->println(F("[ERROR] Failed to initiate jog movement"));
...
Commander::systemCommand_t API_tree[] = {
    ...
    systemCommand("motor", "Motor control:\n"
              ...
              "  motor,stop   - Emergency stop motor movement immediately", cmd_motor),
    ...
    systemCommand("move", "Move motor to position:\n"
              ...
              "  move,rel,X     - Move X millimeters relative to current position (+ forward, - backward)", cmd_move),
    ...
    systemCommand("jog", "Jog motor:\n"
              ...
              "  jog,status    - Display current jog settings", cmd_jog)
};
