...
function raw_data()
	raw_data = if csv_file === nothing
    DataFrame()  # Empty DataFrame
	else
	    IOBuffer(csv_file["data"]) |> data -> CSV.File(data, normalizenames=true, header=2) |> DataFrame
	end
end
...
function clean_data()
	...
	df[!, :DateTime] = DateTime.(string.(df.DATE) .* " " .* string.(df.TIME), "yyyy/mm/dd HH:MM:SS")
	# Calculate the elapsed time in seconds since the first timestamp
	df[!, :ElapsedTime] = Int64.((df.DateTime .- df.DateTime[1]) / Millisecond(1)) / 1000
	# remove :DATE and :TIME column in place
	select!(df, Not([:DATE, :TIME]))
	# move :DateTime and :ElapsedTime to the front
	select!(df, :DateTime, :ElapsedTime, Not([:DateTime, :ElapsedTime]))
	# drop the :DATA_No column
	select!(df, Not([:DATA_No]))
	# rename the columns
	rename!(df, :DM00015 => :P1_PSi, :DM00017 => :P2_PSi, :DM00019 => :P3_PSi, 
:DM00021 => :permeate_volume_current, :W0030 => :Source_air_flow_LPM, :W003B => :Source_Temp_C, :W0038 => :Source_Pressure_Psi, :W003C => :Source_RH, :NU_EP1_1_Current_Pressure_1_ => :P1_raw, :NU_EP1_1_Current_Pressure_2_ => :P2_raw, :NU_EP1_1_Current_Flow_3_ => :P3_raw, :NU_EP1_1_Current_Value_5_ => :permeate_flow_rate)
	# fix the units for colunms
	transform!(df, [:P1_PSi, :P2_PSi, :P3_PSi] .=> ByRow(x -> Float32(x)) .=> [:P1_PSi, :P2_PSi, :P3_PSi])
	transform!(df, [:P1_raw, :P2_raw, :P3_raw] .=> ByRow(x -> x .* 0.0001) .=> [:P1_raw, :P2_raw, :P3_raw])
	transform!(df, :Source_air_flow_LPM => ByRow(x -> x .* 0.1) => :Source_air_flow_LPM)
	transform!(df, :Source_Temp_C => ByRow(x -> x .* 0.1) => :Source_Temp_C)
	transform!(df, :Source_Pressure_Psi => ByRow(x -> x .* 0.1) => :Source_Pressure_Psi)
	transform!(df, :permeate_volume_current => ByRow(x -> x .* 0.1) => :permeate_volume_current)
...
# ╔═╡ e408aef0-4a4a-4e87-b1ea-44db020cb73b
typeof(yaxis)
