...
# ╔═╡ 600f92a6-4889-40f5-977b-daa188b5c3ba
using SHA

# ╔═╡ 1b3c8ab1-3ea1-4199-a2cd-8b7b3ff29f6b
using CSV, DataFrames

# ╔═╡ d3410d1b-5bd2-419d-beb3-e6f645db619b
using StatsBase

# ╔═╡ bd157afe-ddfb-4067-8990-a0ed403fd9ce
using CairoMakie

# ╔═╡ 44f79469-2a67-45a3-8562-43614458a428
using AlgebraOfGraphics

# ╔═╡ ae388ba9-329c-4cf7-a1ed-8a563aa2a31b
md"""
# Analyze and Prepare the Boston Housing Dataset
"""

# ╔═╡ c951fd54-26c1-438a-85a4-d91623d449fc
md"""
## Workflow
"""

# ╔═╡ 62722ae6-796c-4ddd-b535-9630ed1686fb
md"""
1. Use the “housing.data” file from the [UCI repository](https://archive.ics.uci.edu/ml/machine-learning-databases/housing/housing.data). You can learn more about the dataset from the “housing.names” file also available in the [UCI data repository](https://archive.ics.uci.edu/ml/machine-learning-databases/housing/housing.names). You should download the file from the internet using Julia. In your code make sure to download it only if it is not already present in the local folder.
"""

# ╔═╡ 45c25db0-d98f-11ed-24b8-97f507e4665e
import Downloads
...
md"""
2. Check that the file has been fetched correctly by calculating its SHA1 hash and compare it to the following reference value:
```
...
md"""
3. Load data into Julia’s DataFrame. Examine the basic descriptive statistics of the dataset.
"""

# ╔═╡ 499fcd99-cdbd-4b4e-8201-4bf75dfaedda
#housing_data = readlines("housing.data")
housing_data = nothing

# ╔═╡ b8b9ee66-5649-4750-bf6d-8006f34c0086
#housing_names = readlines("housing.names")
...
md"""
4. Identify nominal and continuous variables in the dataset. Create two variables `nominal` and `continuous` that will store vectors holding the column names.
"""
...
md"""
5. Inspect the distribution of nominal variables. To do this, produce tables with the counts of each level for all nominal variables.
"""

# ╔═╡ 2a02fd05-9c94-442a-9197-b8a9321dba3b
countmap(housing_nominal[:, :CHAS])

# ╔═╡ ee3c294c-4ea1-4118-86a8-ff54f5c27489
countmap(housing_nominal[:, :RAD])

# ╔═╡ c74880ef-b0c1-4eab-95df-0809113cf9b1
md"""
6. Check the distributions of continuous variables by producing histograms for each variable. You should produce a plot (or plots) similar to the one below.
"""

# ╔═╡ 65a01362-3448-498a-a09b-a7593fd10afa
md"""
![example plots](https://d16rtcb5cr0vb4.cloudfront.net/C0761+EDA+%28Exploratory+Data+Analysis%29+and+data+preprocessing+with+Julia%29+%2FResources%2FImages%2FM1_histograms_V1.png?Expires=1681423171&Signature=IaV~FnU1ij-mgGUIFqqQ1l-SpUHBRSvjvyF-Jd7ZALZM7YYdoha6rCUnpa1KcJZCDyf8N4PFNiOUpT2T6ij2AO9t9JTO1bT5an0CpWjjHRTyikWdqWJl3VA-8CxIirlYtg8jkF76JbjgW6nlXZ8UtfYWmMHTsKZonf9JmwVz6dbhYqSDVhwS5EpKBMfm5p1shHNBTZsK7H3T1hqcnBm5gq~GdLH8~Q3uSWbuWNXdt55FFe05geGTSPlaYmmahoI60wphQuj76~bndScaKs3UBfU3o-Ux4E783kHTlYk6u9VRo-Q7Bqx3xisKcdE8dR8IMpCgjuMkW9hCPXkOHpuzFQ__&Key-Pair-Id=APKAIHLKH2FX732Z3HGA)
"""
...
md"""
7. The analysis of histograms unveils a pattern in the target (MEDV) variable: there is a strange spike for the highest value of the median house price (confirm in your code that it is 50.0). There are several different ways to handle this situation. For the purpose of this project, remove observations with MEDV equal to 50.0 as we cannot be sure about the quality of this information.
"""

# ╔═╡ 34587e8d-e03c-47f1-ac8c-a3200529102b
atttribute_information = readlines("housing.names")[29:47]

# ╔═╡ 1aeae456-1149-474a-9a3a-1ef4f8ba5295
MEDV_mean = mean(housing_continuous[:, :MEDV])

# ╔═╡ ec496413-4aa3-43e1-8b6d-5eb77a75a79a
MEDV_median = median(housing_continuous[:, :MEDV])

# ╔═╡ 0f77776e-aabe-4250-967d-9eea4a3bdebd
MEDV_maximum = maximum(housing_continuous[:, :MEDV])

# ╔═╡ cba08da3-101b-409d-b3d8-fe3697c9e60e
MEDV_mminimum = minimum(housing_continuous[:, :MEDV])

# ╔═╡ 15f1cd73-0b7e-4717-88c5-1e214793d099
outlier_indicator_vector = housing_continuous[:, :MEDV] .>= MEDV_maximum

# ╔═╡ 2b910af1-4331-48bf-997f-cb1f82421696
...
md"""
8. Produce Kendall’s Tau correlation matrix of variables. We can analyze relationships between all ordinal and continuous features this way. Additionally, Kendall’s correlation assumes that feature relations are monotonic but not necessarily linear, as in the case of Pearson’s correlation. You can check the “Pearson vs Spearman vs Kendall” thread in an online discussion group for more information about main correlation coefficients.
"""

# ╔═╡ cd3f6fff-1dea-4f8e-baa0-51a9c08e7fe5
...
md"""
9. Visualize the correlation matrix as a heatmap, sorting the rows and columns by the correlation with the MEDV target variable. Heatmaps are often used to visualize correlation matrices as color scales to help easily spot the most and least correlated features in the dataset. You should produce a plot similar to the one below.
"""
...
md"""
10. Produce a DataFrame showing all variables and their correlation with MEDV. Sort this DataFrame by absolute value of correlation (to group together the least and most correlated variables).
"""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "01dfd17b28c62b7e78cb61370392556dc1ad60ac"
